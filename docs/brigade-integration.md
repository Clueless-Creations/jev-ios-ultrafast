...
`engine` defaults to `"jev"`. To use the standard model path, pass `engine: "baseline"`, `baselineModel: "openai/gpt-5.4-nano"`, and `minProbability: 0`. The baseline does not supply calibrated confidence. An optional `budgetUsd` sets the CLI's admission budget. The same wrapper, scenario, final label check, and artifact contract apply to either engine. Use the CLI's `compare` command for the [fixed paired benchmark](comparison.md); the wrapper runs one attempt at a time.

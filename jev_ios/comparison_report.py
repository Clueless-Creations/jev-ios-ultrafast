...
def _run(raw):
    ...
    normalized = {
        ...
        "pair": _number(raw.get("pair"), "pair", integer=True, maximum=5),
        ...
        "trace": _relative_path(raw.get("trace"), "trace"),
...
def _manifest(raw):
    ...
    if not isinstance(runs, list) or not 1 <= len(runs) <= MAX_RUNS:
        raise ValueError("Comparison needs one to ten run attempts")
    ...
    if "pairs" in settings:
        count = _number(settings["pairs"], "scheduled pairs", integer=True, maximum=5)
    ...
    if "orders" in settings:
        ...
        if not isinstance(orders, list) or len(orders) > 5 or any(not isinstance(order, list) or len(order) != 2 or any(not isinstance(item, str) for item in order) or sorted(order) != ["baseline", "jev"] for order in orders):
...
<div class="playback"><button class="button" id="play">Play pair</button><button class="button" id="restart">Restart</button><input id="scrub" type="range" min="0" max="100" value="0" step="0.01" aria-label="Playback time"><span class="clock" id="clock">0.0 / 0.0 s</span></div>
<p class="pair-note" id="pair-note"></p><p class="pair-note">Each recording was captured separately. Playback aligns both videos to their runner start and stays at 1×. The shorter video holds its last frame; the source recordings are unsped-up.</p>
...
<div class="table-wrap"><table><thead><tr><th>PAIR / ORDER</th><th>ENGINE / OUTCOME</th><th>FULL RUN</th><th>MEDIAN REQUEST</th><th>ACTIONS / CALLS</th><th>INPUT / OUTPUT TOKENS</th><th>EST. COST</th></tr></thead><tbody id="attempts"></tbody></table></div>
...
data.runs.forEach(r=>{const tr=element('tr');tr.append(element('td','','Pair '+r.pair+' · #'+r.position));const who=element('td');who.append(element('div','run-cell '+(r.backend==='jev'?'lime':'blue'),r.backend==='jev'?'Jev':'Baseline'),element('div','sub',r.model),element('div','sub '+(r.status==='verified'?'':'failure'),r.status+' · '+r.reason));if(r.setup_status!=='ready')who.append(element('div','sub failure','Setup: '+r.setup_status));if(r.artifact_error)who.append(element('div','sub failure','Recording/report: '+r.artifact_error));tr.append(who,element('td','',r.timing_valid?sec(r.elapsed_ms):'Not timed'),element('td','',ms(median(r.model_ms))),element('td','',r.actions_executed+' / '+r.model_calls));const tokens=element('td','',r.input_tokens.toLocaleString()+' / '+r.output_tokens.toLocaleString());if(r.cached_input_tokens)tokens.append(element('div','sub',r.cached_input_tokens.toLocaleString()+' cached input'));tr.append(tokens,element('td','',r.estimated_cost_usd===null?'Unknown':'$'+r.estimated_cost_usd.toFixed(6)));$('attempts').append(tr);});
let videos=[],currentPair=null,duration=0,elapsed=0,startAt=0,playing=false,frame=null,selection=0;
function pause(){playing=false;if(frame)cancelAnimationFrame(frame);videos.forEach(x=>x.video.pause());set('play','Play pair');}
function displayClock(){set('clock',elapsed.toFixed(1)+' / '+duration.toFixed(1)+' s');$('scrub').value=String(elapsed);}
function seek(seconds){elapsed=Math.min(duration,Math.max(0,seconds));videos.forEach(({video,run})=>{const limit=Number.isFinite(video.duration)?Math.max(0,video.duration-.025):Infinity;video.currentTime=Math.min(limit,Math.max(0,seconds+run.video_offset_ms/1000));});displayClock();}
function tick(){if(!playing)return;elapsed=Math.min(duration,(performance.now()-startAt)/1000);displayClock();if(elapsed>=duration){pause();return;}frame=requestAnimationFrame(tick);}
async function play(){if(playing){pause();return;}if(!videos.length)return;const active=selection;if(elapsed>=duration-.02)seek(0);const attempts=videos.filter(({video})=>!video.ended).map(({video})=>{video.playbackRate=1;return video.play().then(()=>true,()=>false);});const started=await Promise.all(attempts);if(active!==selection)return;if(!started.some(Boolean)){set('play','Playback unavailable');return;}playing=true;startAt=performance.now()-elapsed*1000;set('play','Pause');frame=requestAnimationFrame(tick);}
function selectPair(pair){pause();selection++;currentPair=pair;videos=[];$('players').replaceChildren();elapsed=0;const rows=data.runs.filter(r=>r.pair===pair.pair);duration=Math.max(0,...rows.map(r=>(r.elapsed_ms??0)/1000));const activeSelection=selection;document.querySelectorAll('#pairs button').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.pair)===pair.pair)));for(const backend of ['jev','baseline']){const r=rows.find(r=>r.backend===backend),card=element('section','player');const heading=element('div','player-heading');heading.append(element('div','backend '+(backend==='jev'?'lime':'blue'),backend==='jev'?'Jev':'Without Jev'),element('span','status '+(r?.status==='verified'?'verified':''),r?.status||'not attempted'));card.append(heading,element('div','player-meta',r?r.model+' · execution #'+r.position:'No attempt recorded'));const device=element('div','device');if(r?.media_id){const fragment=$(r.media_id).content.cloneNode(true),video=fragment.querySelector('video');video.addEventListener('loadedmetadata',()=>{if(activeSelection!==selection)return;video.currentTime=Math.min(Math.max(0,video.duration-.025),r.video_offset_ms/1000);duration=Math.max(duration,video.duration-r.video_offset_ms/1000);$('scrub').max=String(duration);displayClock();},{once:true});device.append(fragment);videos.push({video,run:r});}else device.append(element('div','empty',r?'No recording attached. This attempt still counts.':'No attempt was recorded for this engine.'));card.append(device);const stats=element('div','player-results');for(const [value,label] of [[r&&r.timing_valid?sec(r.elapsed_ms):'—','full run'],[r?ms(median(r.model_ms)):'—','median request'],[r?r.actions_executed:'—','actions']]){const stat=element('div');stat.append(element('div','small-value',value),element('div','small-label',label));stats.append(stat);}card.append(stats);$('players').append(card);}$('scrub').max=String(duration);$('play').disabled=!videos.length;set('pair-note','Pair '+pair.pair+' · '+(pair.comparable?'Same initial state. ': 'Excluded from speed comparison. ')+pair.reason+(pair.baseline_over_jev!==null?' · Baseline / Jev elapsed time: '+pair.baseline_over_jev.toFixed(2)+'×.':''));displayClock();}
...
$('play').onclick=play;$('restart').onclick=()=>{pause();seek(0);};$('scrub').oninput=event=>{pause();seek(Number(event.target.value));};
...
if(summary.pairs.length)selectPair(summary.pairs[0]);
</script></html>'''
